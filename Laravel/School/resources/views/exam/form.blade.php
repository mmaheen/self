...
@section('content')
    ...
    <body>
        <form action="{{route('exam.form',['id'=>$id])}}" method = "post">
            {{csrf_field()}}
            <table>
                <tr><td>Exam Name</td><td><input type="text" name="name" value = "{{old('name')}}" placeholder = "Exam Name"></td>
                ...
                <tr><td>Invigilator</td><td><select name="teacher_id" id=""><option selected disabled>Teacher</option>@foreach($t as $i)<option value="{{$i->id}}">{{$i->name}}</option>@endforeach</select></td>
                <td>@error('teacher_id')<span class = "text-danger">{{$message}}</span>@enderror</td></tr>
